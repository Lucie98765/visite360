...
$stmt5 = $bdd->prepare(<<<SQL
	SELECT DISTINCT titre_onglet
	FROM Onglet
	WHERE nom_onglet = :value
SQL
);

for ($i=0; $i <sizeof($nom_onglet) ; $i++) { 
	$value = $nom_onglet[$i]['nom_onglet'];
	$result[$value]['nom_onglet']=$value;
	$stmt2->execute(array(':value'=>$value));
	$photo = $stmt2->fetchAll();
	for ($j=0; $j <sizeof($photo) ; $j++) { 
		$result[$value]['photo'][$j]=$photo[$j]['photo'];
	}
	
	$stmt3->execute(array(':value'=>$value));
	$sous_titre = $stmt3->fetchAll();
		for ($j=0; $j <sizeof($sous_titre) ; $j++) { 
		$result[$value]['sous_titre'][$j]=$sous_titre[$j]['texte_sous_titre'];
	}

	$stmt4->execute(array(':value'=>$value));
	$texte = $stmt4->fetchAll();
		for ($j=0; $j <sizeof($texte) ; $j++) { 
		$result[$value]['texte'][$j]=$texte[$j]['texte'];
	}

	$stmt5->execute(array(':value'=>$value));
	$titre = $stmt5->fetch();
	$result[$value]['titre']=$titre['titre_onglet'];
}
